...
if($_POST){
    $case['case_name'] = $_POST['title'];
    $case['case_content1'] = $_POST['editorValue'];
    $case['case_img'] = '/admin/uploads/'.$funs->uploadf_r('pic','uploads');
    $case['case_type'] = $_POST['type'];
    $img_thumb = $case['case_img'];
    // echo $img_thumb.'#####';
   ...
   if($db->insert('cases',$case)){
    ...
    //文字水印图
    $w_path = $funs->watermark('./uploads/'.basename($img_thumb),'水印图片');
        echo "<script>alert('添加案例成功');window.location.href='case_list.php?case_id={$type}';</script>";
    }else{
        echo "<script>alert('添加案例失败，请重新添加');';</script>";
    }
}
